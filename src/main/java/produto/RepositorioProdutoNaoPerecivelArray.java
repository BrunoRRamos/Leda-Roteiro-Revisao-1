...
public class RepositorioProdutoNaoPerecivelArray {
	...
	public boolean existe(int codigo) {
		for (int i = 0; i < produtos.length; i++) {
			if (produtos[i].getCodigo() == codigo && produtos[i] != null) {
				return true;
			}
		}
		return false;
	...
	public ProdutoNaoPerecivel procurar(int codigo) {
		for (int itemIndex = 0; itemIndex < produtos.length; itemIndex++) {
			if (this.produtos[itemIndex].getCodigo() == codigo) {
				return produtos[itemIndex];
			}
		}
		throw new RuntimeException();
	}
